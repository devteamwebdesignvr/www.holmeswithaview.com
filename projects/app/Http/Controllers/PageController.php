...
class PageController extends Controller{
    ...
    function attractionCategory($seo_url){
    ...
    function reviewSubmit(Request $request){
        $validator = Validator::make($request->all(), [
            'email' => 'required|email',  'name'=>"required", "message"=>"required"]);   
        if($validator->fails()){
            ...
            if($request->start_date){
                if($request->end_date){
                    ...
                    $total_night=$day;
                  	...
                  	if($minimum_stay<=$total_night){
                    }else{
                       return response()->json(["message"=>"Minimum stay not matched","status"=>400]);
                    }
                    $main_data=HostAwayAPI::getCalculateReservationPrice($property->host_away_id,$start_date,$end_date,$total_guest);
                    if($main_data['status']=="200"){
                        ...
                        return response()->json(["message"=>"success","status"=>200,"modal_day_view"=>$modal_day_view,"modal_service_view"=>$modal_service_view,"data_view"=>$data_view,"price"=>$price]);
    ...
    public function dynamicDataCategory(Request $request,$seo_url){
      ...
      if($seo_url=="properties"){
              if($request->location_id){
               	 $data1=HostAwayProperty::where("id",$request->location_id)->first();
        		if($data1){
                  return redirect($data1->seo_url);
                }
              }
            }
                ...
                if($request->checkin){
                    if($request->checkout){
                        ...
                        if($ar_gaurav_data){
                            ...
                            unset($data['cvv']);
                            unset($data['expiration-month']);
                            unset($data['expiration-year']);
                            unset($data['firstname']);
                            unset($data['complete']);
                            unset($data['operation']);
                            unset($data['lastname']);
                            unset($data['card_cvv']);
                            unset($data['card_exp_month']);
