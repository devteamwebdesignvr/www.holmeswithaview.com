        ...
        if($data){
            ...
            if($result){
                ...
                foreach(PropertyAmenityGroup::where("property_id",$id)->get() as $c){
                    $am_group=PropertyAmenityGroup::create([
                        "property_id"=>$id1,
                        "status"=>$c->status,
                        "name"=>$c->name,
                        "image"=>$c->image,
                    ...
                    foreach(PropertyAmenity::where("property_amenity_id",$c->id)->get() as $c1){
                        PropertyAmenity::create([
                            "property_amenity_id"=>$am_group->id,
                            "name"=>$c1->name,
                            "status"=>$c1->status,
                            "image"=>$c1->image,
                            "sorting"=>$c1->sorting
        ...
        if($result){
            ...
            if($request->fee_name){
                foreach($request->fee_name as $key => $value){
                    $ar=[
                        "property_id"=>$id,
                        "fee_name"=>$request->fee_name[$key],
                        "fee_rate"=>$request->fee_rate[$key],
                        "fee_type"=>$request->fee_type[$key],
                        "fee_apply"=>$request->fee_apply[$key],
                        "fee_status"=>$request->fee_status[$key]
                    ];
