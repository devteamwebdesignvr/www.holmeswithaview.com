...
class HostFullyPropertyController extends Controller{
    ...
    public function update(Request $request, $id){
        $validator = Validator::make($request->all(), [
            'seo_url' => 'required|unique:host_away_properties,seo_url,'.$id,
        ...
        if($exist){
            ...
            if($request->remove_rental_aggrement_attachment){
              ...
              if ($request->hasFile("banner_image")) {
                Helper::deleteFile($exist->banner_image);
                $data['banner_image'] = Upload::uploadWithLogoImageData($request->file("banner_image"),"properties");
            }
