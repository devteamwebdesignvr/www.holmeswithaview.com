...
class HostAwayAPI{
  ...
  function getPropertyListDataSelect(){
    $data=HostAwayProperty::where(["status"=>"true"])->get();
    $ar=[];
    foreach($data as $d){
      $ar[$d->id]=$d->name;
    }
    ...
    function getBearerToken(){
        ...
        if($token){
            ...
            $client_id=74594;
            $secret_code='7106ddb7749346f2e94880b449b81ad89d9e56e82d18d09e67a2d37eb248a946';
            $curl = curl_init();
    ...
    function createbooking($listing_id,$start_date,$end_date,$amount,$name,$email,$adult,$child,$mobile,$first_name='',$last_name=''){
        $token_data=$this->getBearerToken();
    ...
    function getAllCalendarAPIRun($listingid){
        $curl = curl_init();
        $today=date('Y-m-01');
        $year=date('Y');
    ...
    function getSearchPropertiesList($start_date,$end_date,$guest){
        $new_array=[];
        $token_data=$this->getBearerToken();
        if($token_data['status']=="200"){
            $curl = curl_init();
    ...
    function getSinglePropertyDetail($listing_id){
        $token_data=$this->getBearerToken();
        $curl = curl_init();
        curl_setopt_array($curl, array(
  ...
  function getAllBooking(){
    ...
    function getCalculateReservationPrice($listing_id,$start_date,$end_date,$no_of_guests){
        $token_data=$this->getBearerToken();
        $curl = curl_init();
        curl_setopt_array($curl, array(
    ...
    function getCancelationPolicyData(){
        $token_data=$this->getBearerToken();
        $curl = curl_init();
        curl_setopt_array($curl, array(
        ...
        if ($err) {
            ...
            $fillable=[
                "host_away_id","accountId","name","cancellationPolicyItem",
            ];
            $data= json_decode($response,true);
            if(isset($data['status'])){
                if($data['status']=="success"){
