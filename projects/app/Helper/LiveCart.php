   ...
   function iCalDataCheckInCheckOut($property_id){
    ...
    function iCalDataCheckInCheckOutCheckinCheckout($property_id){
       ...
       // dd($data);
        ...
        if($abc):
            $blocked_date_from_calender=HostAwayDate::where(["status"=>"blocked",'hostaway_id'=>$abc->host_away_id])->get();
        ...
        sort($checkout);
        sort($blocked);
      //  dd($checkin,$checkout,$blocked,$result);
     ...
     function refreshIcalData($property_id,$ical_link,$id){
         IcalEvent::where(['ppp_id'=>$property_id,'event_pid'=>$property_id,
                'ical_link'=>$ical_link])->delete();
